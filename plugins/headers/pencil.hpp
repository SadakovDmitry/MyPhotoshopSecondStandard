...
public:
    PencilTool(vec2i pos_, vec2u size_, wid_t id_, const std::string& file);
...
class PencilAction : public AUndoableAction {
    PencilTool *pencil;
    sfm::vec2i last_pos_ = sfm::vec2i(0, 0);
public:
    PencilAction(PencilTool *pencil_, const IRenderWindow *render_window_, const Event &event_);
